...
static seq_t *rdr_pattok2seq(rdr_t *rdr, const tok_t *tok) {
    const uint32_t T = tok->len; // length of the sequence.
    ...
    seq_t *seq = xmalloc(sizeof(seq_t) + sizeof(pos_t) * T);
    info("inside rdr_pattok2seq:\n");
    info("the number of unigram features is %d, bigram is %d\n", rdr->nuni, rdr->nbi);
    ...
    for (uint32_t t = 0; t < T; t++) {
        ...
        info("seq->pos at %d:\n", t);
        info("lbl, uobs, bobs, tmp are %u, %u, %u, %u\n", seq->pos[t].lbl, seq->pos[t].uobs, seq->pos[t].bobs, tmp);
    }
    ...
    if (tok->lbl != NULL) {
        for (uint32_t t = 0; t < T; t++) {
            ...
            uint64_t id = qrk_str2id(rdr->lbl, lbl);
            seq->pos[t].lbl = id;
            info("id(lbl) at position %u is %u\n", t, id);
        }
    }
    return seq;
}
...
seq_t *rdr_raw2seq(rdr_t *rdr, const raw_t *raw, bool lbl) {
    ...
    for (uint32_t t = 0; t < T; t++) {
        ...
        // debugging
        info("inside rdr_raw2seq:\n");
        for (uint32_t iter = 0; iter <= strlen(line) / 2 + 1; iter++) {
            info("toks %d is %s \n", iter, toks[iter]);
        }
        // end debugging
...
seq_t *rdr_readseq(rdr_t *rdr, FILE *file, bool lbl) {
    ...
    struct pos_s one_seq = seq->pos[0];
    info("seq is %d, %d, %d, %d, %d\n", one_seq.lbl, one_seq.ucnt, one_seq.bcnt, one_seq.uobs, one_seq.bobs);
    rdr_freeraw(raw);
    return seq;
}
